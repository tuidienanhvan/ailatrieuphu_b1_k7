...
export const usePlayScreen = () => {
  ...
  const tierId = getTierByLevel(currentLevel);
  const currentTier = GAME_CONFIG.tiers.find(t => t.id === tierId) || GAME_CONFIG.tiers[0];
  ...
  const handleGameOver = useCallback((reason: 'wrong' | 'timeout' | 'stop' | 'victory') => {
    ...
    if (reason === 'victory') {
      prize = getPrizeAmount(GAME_CONFIG.totalLevels);
      levelToSave = GAME_CONFIG.totalLevels;
      gameResult = 'victory';
      wrongAnswerLevel = null;

      logEvent('GAME_END', { result: 'VICTORY', finalLevel: GAME_CONFIG.totalLevels, prize: prize });
      setGameState(GameState.VICTORY);
      playSound('win');
    } else if (reason === 'stop') {
      prize = getPrizeAmount(currentQIndex);
      ...
      // wrong or timeout
      const safeIndex = calculateFallingPrizeIndex(currentQIndex, GAME_CONFIG.milestones);
      prize = getPrizeAmount(safeIndex);
      levelToSave = safeIndex;
      gameResult = 'gameover';
  ...
  const handleStopGame = useCallback(() => {
    openModal('stop', { stopAmount: getPrizeAmount(currentLevel) });
