...
import { usePlayScreen } from '@game/hooks/play/usePlayScreen';
...
// Events
import { SystemErrorOverlay } from '@game/events/pranks/system-error/SystemErrorOverlay';
import { useSystemError } from '@game/events/pranks/system-error/useSystemError';
...
export const PlayScreen: React.FC = () => {
    ...
    return (
        <div className="flex-1 flex flex-row w-full h-full relative overflow-visible animate-fade-in bg-black">
            ...
            {showSystemError && <SystemErrorOverlay />}

            {/* --- CONTENT LAYERS --- */}
            <div className="flex-1 flex flex-col relative z-10 h-full">
                {/* PRANK: Hide content only during logic glitch state */}
                <div className="flex flex-col w-full">
                    ...
                    <div className="mt-4">
                        <QuestionBoard
                            timer={timer}
                            maxDuration={GAME_CONFIG.defaultTimerDuration}
                            hideContent={isGlitched}
                        />
                    </div>
                </div>
            ...
            {/* --- SIDEBAR PRIZES --- */}
            <SidebarPrizes prizes={GAME_CONFIG.prizes} currentLevel={currentLevel} currentTier={1} />

            {/* --- MODALS (Imported Directly) --- */}
            <PhoneModal
                isOpen={activeModal === 'phone'}
                onSelect={(id) => {
                    const helper = GAME_CONFIG.phoneHelpers.find((p: any) => p.id === id);
