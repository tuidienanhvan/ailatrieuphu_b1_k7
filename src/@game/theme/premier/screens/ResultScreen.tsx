...
export const ResultScreen: React.FC<ResultScreenProps> = ({
    ...
    // Theme configuration
    // Victory OR Tier Complete uses the Gold Theme
    const isSuccess = isVictory || isTierComplete;
    ...
    const theme = isSuccess
        ? {
            ...
            title: isVictory ? 'CHIẾN THẮNG' : `HOÀN THÀNH TIER ${tierNumber}`,
            icon: Trophy,
            subtext: isVictory ? 'BẠN LÀ TRIỆU PHÚ MỚI' : 'SẴN SÀNG CHO THỬ THÁCH MỚI?'
        }
        : {
    ...
    return (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/95 animate-fade-in overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none">
                {/* Rotating Rays (Only for Success) */}
                {isSuccess && (
                ...
                {/* Ambient Glow */}
                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full blur-[100px] opacity-20 ${isSuccess ? 'bg-yellow-500' : 'bg-red-600'}`}></div>
            ...
            <div className="relative w-full max-w-4xl transform transition-all duration-500 hover:scale-[1.01]">
                ...
                <div className={`relative bg-[#020617] border-2 border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col items-center p-12 lg:p-16 ${theme.glow}`}>
                    ...
                    <div className="text-center z-10 mb-10">
                        <h2 className="text-[80px] leading-normal py-4 font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white via-slate-200 to-slate-400 drop-shadow-sm mb-2 font-sans">
                            {theme.title}
                        </h2>
                    ...
                    <div className="relative w-full max-w-2xl h-40 mb-12 flex items-center justify-center z-10">
                        ...
                        <div className="relative flex flex-col items-center">
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-[0.5em] mb-1">
                                {isTierComplete ? 'PHẦN THƯỞNG ĐÃ ĐẠT' : 'TỔNG GIẢI THƯỞNG'}
                            </span>
                            <span
                                className="text-6xl md:text-7xl font-mono font-black tracking-tighter drop-shadow-2xl"
                                style={{
                                    color: isSuccess ? '#fffbeb' : '#fff',
                    ...
                    <div className="flex gap-6 z-20">
                        ...
                        <button
                            onClick={onReset}
                            className={`group relative px-10 py-4 bg-transparent overflow-hidden rounded-full transition-all duration-300 active:scale-95 border border-white/20 hover:border-white/40 ${isTierComplete ? '' : 'px-16 py-6'}`}
                        >
                            <div className="absolute inset-0 bg-white/5 group-hover:bg-white/10 transition-colors"></div>
                            <div className="relative flex items-center gap-2 text-white/80 font-bold uppercase tracking-widest text-sm">
                                <RotateCcw size={18} />
                                <span>Chơi Lại</span>
                            </div>
                        </button>

                        {/* Continue Button (Tier Complete only) */}
                        {isTierComplete && onContinue && (
                            <button
                                onClick={onContinue}
                                className="group relative px-16 py-6 bg-transparent overflow-hidden rounded-full transition-all duration-300 active:scale-95"
                            >
                                ...
                                <div className="relative flex items-center gap-3 text-black font-black uppercase tracking-[0.2em] text-xl">
                                    <span>TIẾP TỤC</span>
                                    <Trophy size={24} strokeWidth={3} className="group-hover:rotate-12 transition-transform duration-500" />
                                </div>
                            </button>
                        )}
                    </div>
