...
// 52. AudioContext Singleton Class
class AudioManager {
  ...
  // 50. Pause/Resume for Tab Visibility
  ...
  playSound(type: SoundType) {
    try {
      ...
      const createOsc = (freq: number, oscType: OscillatorType, startTime: number, duration: number, volume: number) => {
      ...
      // Sound Definitions kept same as original...
      switch (type) {
        case 'start':
            [261.63, 329.63, 392.00, 523.25].forEach((f, i) => createOsc(f, 'triangle', now + i * 0.1, 1, 0.2));
            break;
        case 'select':
            createOsc(880, 'sine', now, 0.1, 0.1);
            break;
        case 'correct':
            createOsc(523.25, 'sine', now, 0.6, 0.2);
            createOsc(659.25, 'sine', now + 0.1, 0.6, 0.2);
            createOsc(783.99, 'sine', now + 0.2, 0.8, 0.2);
            break;
        case 'wrong':
            ...
            break;
        case 'lifeline':
            for(let i=0; i<8; i++) createOsc(1000 + i * 200, 'sine', now + i * 0.05, 0.3, 0.1);
            break;
        case 'tick':
        ...
        case 'win':
            ...
            for(let j=0; j<3; j++) chords.forEach((f, i) => createOsc(f, 'triangle', now + j * 0.6 + i * 0.1, 0.5, 0.15));
            break;
        case 'lose':
            [220, 207, 196, 185].forEach((f, i) => createOsc(f, 'sawtooth', now + i * 0.2, 0.8, 0.1));
            break;
      }
