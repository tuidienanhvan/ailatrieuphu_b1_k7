...
export const SidebarPrizes: React.FC<SidebarPrizesProps> = React.memo(({ prizes, currentLevel, currentTier }) => {
    // Lọc prizes theo tier hiện tại (mỗi tier có 15 levels)
    const tierPrizes = useMemo(() => {
        const startLevel = (currentTier - 1) * 15 + 1;
        const endLevel = currentTier * 15;
        return prizes.filter(p => p.level >= startLevel && p.level <= endLevel);
    }, [prizes, currentTier]);

    const displayPrizes = useMemo(() => [...tierPrizes].reverse(), [tierPrizes]);
            ...
            <div className="relative w-[380px]">
                ...
                <div className="relative flex flex-col gap-1 z-10 px-5 pb-6">
                    {displayPrizes.map((prize) => {
                        ...
                        const isJackpot = prize.level === currentTier * 15; // Jackpot của tier hiện tại
