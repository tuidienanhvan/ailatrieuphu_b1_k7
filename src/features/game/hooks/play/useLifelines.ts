...
export const useLifelines = (
  ...
  const usedInRound = useGameStore(s => s.usedInRound); // Get used items
  ...
  const handle5050 = useCallback(() => {
    // Check if available AND not used in this round yet
  ...
  const handleChangeQuestion = useCallback(() => {
    ...
    playSound('lifeline');
  }, [lifelines.changeQuestion, selectedAnswer, backupQuestions, replaceCurrentQuestion, useLifeline, startTimer, usedInRound, logEvent]);
  ...
  const handlePhoneSelect = useCallback((personName: string) => {
    ...
    useLifeline('phone');
    logEvent('USE_LIFELINE', { type: 'PHONE_FRIEND', person: personName, suggestion: suggest });
    playSound('lifeline');
  }, [gameQuestions, currentLevel, openModal, useLifeline, logEvent]);
  ...
  const handleAskAI = useCallback(() => {
    ...
    // LOGIC XÁC SUẤT:
    // Cơ bản: 50% đúng.
    // Chuyên gia: 99.8% đúng.
    const accuracy = hasExpertAI ? 0.998 : 0.50;
    
    const isRight = Math.random() < accuracy; 
    
    // Nếu đúng -> chọn đáp án đúng. Nếu sai -> chọn random 1 trong 3 đáp án sai.
    ...
    // Fake % confidence để hiển thị cho người chơi tin tưởng (dù máy có thể sai ở chế độ cơ bản)
    const percent = hasExpertAI 
        ? (99 + Math.random()).toFixed(1) // 99.0 - 100.0%
        : (65 + Math.random() * 25).toFixed(1); // 65.0 - 90.0% (Máy cơ bản thường tự tin ảo)
